...
const sources={allergy:{title:'BAK-104 · Allergen information',text:courses[3].allergen},fees:{title:'Demo catalogue · Course fees',text:'Kitchen Essentials (CUL-210) is S$160. This is the lowest fee in this sample catalogue.'},policy:{title:'Course guide example · Refund policy',text:'For cancellation 5 days before the start, the course guide gives a 50% refund or a free transfer. Confirm the current policy with the academy.'}};
...
if(c)return{text:`${c.name} (${c.id}) costs S$${c.fee}. Campus: ${c.campus}. Duration: ${c.duration}. These are demonstration catalogue details.`,sources:['catalogue:'+c.id]};
return{text:'Sorry, I could not find that information in the demo course documents. Please contact the academy so a member of staff can help.',sources:[]};}
function validate(d){const errors={};if(!String(d.name||'').trim())errors.name='Enter a sample learner name.';if(!/^[89]\d{7}$/.test(String(d.mobile||'').trim()))errors.mobile='Enter an 8-digit Singapore mobile number starting with 8 or 9.';if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(d.email||'')))errors.email='Enter a valid email address, such as learner@example.com.';if(!d.consent)errors.consent='Please agree before continuing.';return errors;}
